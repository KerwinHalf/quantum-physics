...
const examData = [
    {
        title: " 密度矩阵一：基础巩固与密度矩阵入门",
        questions: [
            {
                q: "1. 关于纯态与混态：一个处于混态的系统（例如 50% $|1\\rangle$ 和 50% $|2\\rangle$），如果我们站在一个非常巧妙的观测角度（选择一个特殊的基底），是有可能把它看成一个纯态的。",
                a: "判断：错 (False)",
                detail: "<strong>理由：</strong>混态的本质是信息的缺失（熵 > 0）。数学上，幺正变换（改变基底）不改变矩阵的迹和纯度 $\\text{Tr}(\\rho^2)$。物理上，“两根独立的棍子”永远变不成“一根棍子”。",
                note: "点评：这是铁律！基变换只能改变观测结果的分布，改变不了系统的本质纯度。"
            },
            {
                q: "2. 关于相干性：已知某系统的密度矩阵在能量表象下是对角矩阵 $\\rho = \\begin{pmatrix} 0.6 & 0 \\\\ 0 & 0.4 \\end{pmatrix}$。这说明该系统处于定态，且粒子在两个能级之间完全没有转化（跃迁）。",
                a: "判断：对 (True)",
                detail: "<strong>理由：</strong>非对角项（相干项）为 0，说明没有“桥”；且哈密顿量是对角的，没有“推手” ($H_{12}=0$)。所以布居数 $\\rho_{nn}$ 不随时间变化。",
                note: "点评：没有路（相干项），也没有车（相互作用），粒子只能原地踏步。"
            },
            {
                q: "3. 关于旋波近似 (RWA)：在推导 Rabi 振荡时，我们扔掉了 $e^{i(\\Omega+\\omega)t}$ 这一项（反旋项），是因为这一项违反了能量守恒定律，物理上绝对不允许发生。",
                a: "判断：错 (False) 或 不严谨",
                detail: "<strong>理由：</strong>量子力学允许短时间的能量不守恒（不确定性原理）。真正的原因是它是“快变项”！在积分演化时，正负快速抵消，平均效果为零，对长时间的累积演化没有实质贡献。",
                note: "点评：记住四个字——“瞎忙活”！频率太快，干不了实事。"
            },
            {
                ...
                note: "点评：这就是“棍子模型”！高度（对角元）不变，但在赤道面上绕 Z 轴疯狂转圈（进动）。"
            },
            {
                ...
                note: "点评：只要整体是纠缠的，局部一定是混的（丢失了信息）。"
            },
            {
                ...
                note: "点评：这就是“合理的买卖”！我有钱买，你有货卖，能量守恒，交易成功。"
            }
        ]
    },
{
        title: "密度矩阵二：进阶动力学与纠缠本质",
        questions: [
            {
                ...
                note: "点评：想要变纯？必须把不需要的信息“扔掉”（测量/耗散），孤立演化是做不到的。"
            },
            {
                ...
                note: "点评：只有“单身”（直积态）的人，心里才只有自己（局部纯态）；“恋爱”（纠缠）的人，心里乱糟糟的（局部混态）。"
            },
            {
                ...
                note: "点评：因为 $H$ 只有非对角项（推手），没有对角项（楼层差），所以粒子像荡秋千一样在两态之间完美震荡。"
            },
            {
                ...
                note: "点评：当 $\\alpha=\\beta$ 时，这就是完全混态。纠缠越深，局部越乱。"
            }
        ]
    },
{
        title: "密度矩阵三：高阶计算与物理本质（补全版）",
        questions: [
            {
                ...
                note: "点评：这就叫“殊途同归”。不同的制备历史，可以导致完全相同的物理状态（密度矩阵）。"
            },
            {
            ...
            },
            {
                ...
                note: "点评：<strong>非对角项不为 0！</strong> 这说明即使扔掉了 A，粒子 B 内部依然保留了部分相干性（不是完全混态）。这是 W 态的特性。"
            }
        ]
    },
{
        title: "密度矩阵四：熵、测量与热力学（终极篇）",
        questions: [
            {
                ...
                note: "点评：熵衡量的是“无知程度”。纯态我们全知道（无知=0），完全混态我们全瞎蒙（无知=最大）。"
            },
            {
                ...
                note: "点评：高温导致最大无序。能量差微不足道，粒子去哪儿的概率都一样。"
            },
            {
                ...
                note: "点评：这就是所谓的“波函数坍缩”在密度矩阵语言里的描述——非对角项（相干性）的瞬间消失。"
            }
        ]
    }
];